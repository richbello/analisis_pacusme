...
USUARIOS = {
    "admin": {
        "password": "admin123",
        "rol": "Administrador",
        "permisos": ["seguridad", "audio", "bodega", "generar_plantilla", "auditar", "analisis_pac"]
    },
    "contador": {
        "password": "contador2025",
        "rol": "Contador",
        "permisos": ["seguridad", "generar_plantilla", "analisis_pac"]
    },
    "auxiliar": {
        "password": "aux123",
...
class AnalizadorPAC:
    ...
    def exportar_excel(self):
        ...
        ws2['A1'] = 'ANÁLISIS POR PERÍODO PRESUPUESTAL'
        ...
        # Hoja 3: Por Centro Gestor
        ws3 = wb.create_sheet('Por Centro Gestor')
        ws3['A1'] = 'ANÁLISIS POR CENTRO GESTOR'
        ...
        # Hoja 4: Por Fondos
        ws4 = wb.create_sheet('Por Fondos')
        ws4['A1'] = 'ANÁLISIS POR TIPO DE FONDOS'
        ...
        # Hoja 5: Interpretaciones
        interpretaciones_df = self.generar_interpretaciones()
        ws5 = wb.create_sheet('Interpretaciones')
        ws5['A1'] = 'INTERPRETACIÓN Y HALLAZGOS DEL ANÁLISIS'
        ...
        for row in ws5.iter_rows(min_row=4, max_row=ws5.max_row):
            ws5.row_dimensions[row[0].row].height = 45
...
st.set_page_config(
    page_title="Alcaldía Local de Usme - Sistema de Gestión",
    page_icon="🏛️",
    layout="wide",
...
st.caption("Sistema Integrado de Gestión • Seguridad / Audio / Bodega / Análisis PAC")
...
tab1, tab2, tab3, tab4 = st.tabs(["🛡️ Seguridad", "📊 Análisis PAC", "🔊 Audio", "📦 Bodega"])
...
with tab1:
    if not st.session_state.logged_in:
        ...
        st.info("Por favor, inicia sesión desde el panel izquierdo para acceder a esta sección.")
    else:
        ...
        <div class="info-card">
            ...
            <p><strong>Usuario:</strong> {st.session_state.user}</p>
            <p><strong>Rol:</strong> {st.session_state.rol}</p>
            <p><strong>Permisos:</strong> {', '.join(st.session_state.permisos)}</p>
        </div>
        """, unsafe_allow_html=True)
...
with tab2:
    if not st.session_state.logged_in:
        ...
        <div class="info-card">
            ...
            <p>Genera análisis completo del Reporte PAC con:</p>
            <ul>
                <li>📅 Resumen por período presupuestal</li>
                <li>🏢 Análisis por centro gestor</li>
                <li>💰 Distribución por fondos</li>
                <li>📈 Métricas de ejecución y disponibilidad</li>
                <li>⚠️ Detección de sobregiros</li>
                <li>🔍 Interpretaciones automáticas</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
                    ...
                    with st.spinner("⚡ Procesando análisis... Por favor espera."):
                        try:
                            ...
                            result_tab1, result_tab2, result_tab3, result_tab4 = st.tabs([
                                "📅 Por Período", "🏢 Por Centro", "💰 Por Fondos", "🔍 Interpretaciones"
                            ])
...
with tab3:
    st.subheader("🔊 Módulo de Audio")
    
    if not st.session_state.logged_in:
        st.warning("🔒 Debes iniciar sesión para acceder a este módulo")
    elif not tiene_permiso("audio"):
        st.error(f"❌ Tu rol ({st.session_state.rol}) no tiene acceso a este módulo")
    else:
...
with tab4:
    st.subheader("📦 Módulo de Bodega")
    
    if not st.session_state.logged_in:
        st.warning("🔒 Debes iniciar sesión para acceder a este módulo")
    elif not tiene_permiso("bodega"):
        st.error(f"❌ Tu rol ({st.session_state.rol}) no tiene acceso a este módulo")
    else:
        st.info("📦 Módulo en desarrollo")
...
<div style='text-align: center; color: gray; padding: 20px;'>
    <p>🏛️ <strong>Alcaldía Local de Usme</strong> • Sistema Integrado de Gestión</p>
